...
class CpuTimeSlice(object):
    ...
    def __init__(self, free_processors, start_time, duration, total_processors):
        ...
        self.job_ids = set() #used to create empty set as empty () means empty dictionary
    ...
    def addJob(self, job):
	print "add Job ",job.id, " job procc need:", job.num_required_processors, "slice start:", self.start_time
	print "before addn: free proc  ", self.free_processors, "  ", self.job_ids
	...
	print "Remaining proc: ",self.free_processors, "  ", self.job_ids

    def delJob(self, job):
        assert job.num_required_processors <= self.busy_processors, job
        self.free_processors += job.num_required_processors
	print "del Job",job.id
