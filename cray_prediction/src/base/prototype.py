...
class JobSubmissionEvent(JobEvent): pass	#TODO: Ask Prakash about the job event order.
...
class Job(object):
    def __init__(self, id, user_estimated_run_time, actual_wait_time, actual_run_time, num_required_processors,num_requested_processors,
            submit_time=0, admin_QoS=0, user_QoS=0, user_id=0,group_id=0,queue_id=0): # TODO: are these defaults used?
	#print "inside prototype.py inside class job(object)"
	...
	self.actual_wait_time = actual_wait_time
        ...
        self.num_requested_processors = num_requested_processors
	...
	self.queue_gpu = {}
	self.queue_xphi = {}
	self.queue_cpu.update({self.id:self.queue_id})
	"""
	if self.queue_id == 0 or self.queue_id == 1 or self.queue_id == 2 or self.queue_id == 3:
		 self.queue_group = 0
	elif self.queue_id == 5 or self.queue_id == 6:
		 self.queue_group = 1
	elif self.queue_id == 7:
		 self.queue_group = 2
        #print "user id from prototype",self.user_id
        # not used by base
        self.submit_time = submit_time # Assumption: submission time is greater than zero
        self.start_to_run_at_time = self.submit_time+self.actual_wait_time  # changed from -1 to this by kruthika
	#self.start_to_run_at_time = -1
        ...
        self.start_time = self.submit_time+self.actual_wait_time
        self.end_time = self.submit_time+self.actual_wait_time+self.actual_run_time
    ...
    def __repr__(self):
        return type(self).__name__ + "<id=%(id)s,actual_wait_time=%(actual_wait_time)s, start_time=%(start_time)s,end_time=%(end_time)s, user_estimated_run_time=%(user_estimated_run_time)s, actual_run_time=%(actual_run_time)s, required_processors=%(num_required_processors)s, start_to_run_at_time=%(start_to_run_at_time)s, submit_time=%(submit_time)s>, user_id=%(user_id)s" % vars(self)
...
class StupidScheduler(object):
	...
	#print "inside prototype.py inside handlesubmissionofjobevent"
...
class Machine(object):
    ...
    def __init__(self, event_queue, wait_predictor, run_predictor, scheduler):
	#print "inside prototype.py inside class Machine"
        self.event_queue = event_queue
        self.event_queue.add_handler(JobStartEvent, self._start_job_handler)
	#print "Hi from machine"
        self.waitTimePredictor = wait_predictor
        self.runTimePredictor = run_predictor
	self.scheduler = scheduler

    def _start_job_handler(self, event):
        assert type(event) == JobStartEvent
	self.scheduler.cpu_snapshot.assignJob(event.job, event.timestamp)  #change sid
    ...
    def _add_job(self, job, current_timestamp):
        # print "actualrun:",job.actual_run_time
        #print "estimated:",job.user_estimated_run_time
        #assert job.actual_run_time  <= job.user_estimated_run_time
        #self.event_queue.RunningJobs.append(1)
        ...
        #print job.id,"\t","rr from prototype:",self.event_queue.RunningJobs
        if self.waitTimePredictor != None:
	    #print job.id
	    #print self.event_queue.RunningJobs
            self.waitTimePredictor.notify_job_start_event(job, self.event_queue.QueuedJobs, self.event_queue.RunningJobs)
	
        if self.runTimePredictor != None:
            self.runTimePredictor.notify_job_start_event(job)
...
class ValidatingMachine(Machine):
    ...
    machine usage
    """
    def __init__(self, num_processors, event_queue, wait_predictor,run_predictor, scheduler):
	#print "inside prototype.py inside class validatingmachine(object)"
        super(ValidatingMachine, self).__init__(event_queue, wait_predictor,run_predictor, scheduler)
    ...
    def _add_job(self, job, current_timestamp):
	#print "inside prototype.py inside class validatingmachine(object) add job"
        #assert job.num_required_processors <= self.free_processors
	
	#print "job coming for running\t",job
        self.jobs.add(job)
        super(ValidatingMachine, self)._add_job(job, current_timestamp)

    def _remove_job_handler(self, event):
	#print "inside prototype.py inside class validatingmachine(object) remove job handler"
...
def _job_input_to_job(job_input, total_num_processors):
    #print "inside prototype.py inside _job_input_to_job"
    # if job input seems to be problematic  
    if job_input.run_time <= -2 or job_input.num_allocated_processors <= 0 or job_input.wait_time < 0 or job_input.submit_time < 0:  #sid
        return Job(
            id = job_input.number,
            user_estimated_run_time = 1,
            actual_wait_time = 0,
	    #actual_run_time = float(max(job_input.run_time,1)), 
	    ...
	    queue_id = job_input.queue_number,
            num_requested_processors = job_input.num_requested_processors 
        )

    return Job(
        id = job_input.number,
        #user_estimated_run_time = int(max(job_input.requested_time, job_input.run_time, 1)), #TODO: directly take the user Requested runtime
	user_estimated_run_time = int(job_input.requested_time),
        #actual_wait_time = int(max(job_input.wait_time, 0)),
        actual_wait_time = job_input.wait_time,
        #actual_run_time = float(max(job_input.run_time,1)), #TODO: Change the actual runtime #sid
	actual_run_time = int(job_input.run_time),
...
def _job_inputs_to_jobs(job_inputs, total_num_processors):
	...
	#print "job_input inside protoype.py"
	#print job_input
