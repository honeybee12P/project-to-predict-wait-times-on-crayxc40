...
class Plain_Weighted_Predictor:
	...
	def get_prediction_plain(self, individual_distances, individual_wait_times, job_id, weights):
		...
		self.my_predictions[job_id] = predicted_wait
                #if job_id == 60429:
			#print "validation:\t","prediction for 60429 coming from Plain weighted predictor\t",predicted_wait
	...
	#get_prediction_function
		...
		self.prediction_list.sort() #this will sort on first value i.e. overall_distance
                ...
                if go_ahead:
                    ...
                    #if job_id == 60429:
			#print "validation\t","prediction for 60429 coming from DBScan\t",predicted_wait
...
class Batch_System:
	...
	def __init__(self):
		...
		self.queue_running_jobs = {}		#cray_addition_sid
                ...
                self.sum_test_min = [float('inf')]*10
                self.sum_test_max = [-float('inf')]*10
                self.queue_id_map = {}
		"""
	...
	def __set_job_properties(self, job_id, reqsize, ert):
		...
		self.queue_waiting_jobs[job_id] = 0.0		#cray_addition_sid
		self.queue_running_jobs[job_id] = 0.0		#cray_addition_sid
		...
		for wjob in self.__waiting_job_id:
			...
			if self.__queue_group[wjob] == q:
				...
				self.queue_waiting_jobs[job_id] += 1
		...
		for rjob in self.__running_job_id:
			...
			if self.__queue_group[rjob] == q:
				...
				self.queue_running_jobs[job_id] += 1
		...
		if job_id == 56815:
			...
			print "validation: PMAX: job_id=",job_id," PMAX=",self.user_proc_features_max
                ...
                sum_list = [self.sum_wait_ert[job_id], self.sum_wait_req[job_id], self.sum_wait_elapsed[job_id], self.sum_run_ert[job_id], self.sum_run_req[job_id], self.sum_run_elapsed[job_id], self.sum_higher_priority_jobs[job_id], self.sum_starving_jobs[job_id], self.queue_waiting_jobs[job_id], self.queue_running_jobs[job_id]]
                ...
                self.job_feature_vector[job_id] = [self.__req_size[job_id],
						   ...
						   self.queue_waiting_jobs[job_id],
						   self.queue_running_jobs[job_id]]
		if job_id == 56815:
	...
	def __get_qwt_strategy_based(self, job_id, wait_time):      #wait_time is wait time of job_id
		...
		correlation_targets = []
                ...
                for j in self.__history_jobs:	
				    ...
				    this_feature_list.append(self.queue_waiting_jobs[h_id])
				    this_feature_list.append(self.queue_running_jobs[h_id])
		...
		for j in self.__history_jobs:	
			if self.__arr_time[job_id] >= (self.__arr_time[j] + self.__wait_time[j]):
				...
				UR_demand_ert_distance = float(abs(feature_name[job_id]-feature_name[j]))/float(self.user_proc_features_max[feature_id]-self.user_proc_features_min[feature_id])
				    ...
				    distance.append(float(abs(self.queue_waiting_jobs[job_id]-self.queue_waiting_jobs[j])/float(self.sum_test_max[8]-self.sum_test_min[8])))

				    distance.append(float(abs(self.queue_running_jobs[job_id]-self.queue_running_jobs[j])/float(self.sum_test_max[9]-self.sum_test_min[9])))
		...
		predicted_wait = 0
		    ...
		    #printValidation
			    ...
			    #if job_id == 60429:
 			    	#print "validation:\t", job_id, "\t","Regression output for 60429:",regression_output
				...
				#if job_id == 60429:
 			    		#print "validation:\t", job_id, "\t","WA predictor output for 60429:", predicted_wait
                            else:
                                predicted_wait = 10.0
	...
	def convert_logs(self, input_file_name):		
        ...
        def notify_arrival_event(self, inJob, waiting_jobs, running_jobs):
	    ...
	    #if inJob.id == 57051:
	    	#print "Validation:\tWaiting jobs for 56815\t",self.__waiting_job_id
            ...
            #if inJob.id == 56815:
	    	#print "Validation:\tWaiting jobs ert for 56815\t",self.__waiting_job_ert
            ...
            #if inJob.id == 56815:
	    	#print "Validation:\tWaiting jobs reqsize for 56815\t",self.__waiting_job_reqsize
	    ...
	    #	print "validation\t queue id for 53706\t", self.__queue_id[inJob.id]
            if inJob.id == 56815:
	    	print "validation\t queue group for 56815\t", self.__queue_group[inJob.id]
	    ...
	    #	print "validation\treal run time for 53706\t",  self.__real_run_time[inJob.id]
            ...
            if inJob.id == 56815:
	    	print "validation\tuser id for 56815\t", self.__user_id[inJob.id]
		...
		#print "validation\tReal run time\t",self.__real_run_time[inJob.id]
            ...
            self.__wait_time[inJob.id] = inJob.start_to_run_at_time - inJob.submit_time
	    #if inJob.id == 60429:
        ...
        def notify_job_start_event(self, inJob, waiting_jobs, running_jobs):
            #print "hi coming from notify job start event"
            self.__running_job_id = [job.id for job in running_jobs]
	    #print inJob.id,"\t",self.__running_job_id
	    #if inJob.id == 57051:
            	#print inJob.id,"Running Jobs\t",self.__running_job_id
            self.__waiting_job_id =  [job.id for job in waiting_jobs]
	    #if inJob.id == 60429:
            ...
            if len(self.__history_jobs) >10000:
               #self.__history_jobs.insert(0,inJob.id)
